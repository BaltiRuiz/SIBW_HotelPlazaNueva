<?php
    class ContenedorHabitacionesSQL{
        ...
        function ContenedorHabitacionesSQL(){
            $this->seleccion = "SELECT habitaciones.* FROM habitaciones INNER JOIN menudinamico ON tipohabitacion = idmenu WHERE numhabitacion = 1 order by orden ASC" ;
    ...
    class ContenedorActividadesSQL{
        ...
        function ContenedorActividadesSQL(){
            $this->seleccion = "SELECT actividades.* FROM actividades INNER JOIN menudinamico ON nombreactividad = idmenu order by orden ASC" ;
            $this->resultado = mysql_query($this->seleccion) ;
            $this->numfilas = mysql_num_rows ($this->resultado) ;
        }
            ...
            <div id="primarycontent">
                <form action="" method="POST">
                    <fieldset>
                        ...
                        Password: <input type="password" name="pass" size="32" required ><br><br>
                        <input type="submit" value="Enviar">
                    </fieldset>
                </form>

                <?php
                if(isset($_POST["nombre"]) && isset($_POST["apellidos"]) && isset($_POST["telefono"]) && isset($_POST["email"]) && isset($_POST["DNI"]) && isset($_POST["pass"])){
                    $ntarjeta = NULL ;
                    $nom = $_POST["nombre"] ;
                    ...
                    $correorepetido = mysql_query("SELECT * FROM usuarios WHERE correo = '".$email."'") ;
                    $numfilasrepetido = mysql_num_rows($correorepetido) ;

                    if ($numfilasrepetido == 1){
                        echo '<script type="text/javascript">alert("¡¡El correo introducido ya existe en el sistema!!");</script>' ; 
                    }
                    else{
                        mysql_query("INSERT INTO usuarios (nombre, apellidos, dni, correo, numtarjeta, password, telefono) 
                        VALUES ('$nom', '$apellidos', '$DNI', '$email', '$ntarjeta', '$password', '$telefono')") ;
...
<?php if($seccion == "ConsultarHab"){
    ...
    if(isset($_GET["fentrada"]) && isset($_GET["fsalida"])){
        ...
        $entradaaux = new DateTime($entrada) ;
        $salidaaux = new DateTime($salida) ;
        $intervalo = date_diff($salidaaux, $entradaaux) ;
        ...
        if ($salida <= $entrada){
            ...
            if (isset($_SESSION["idreserva"])){
                ...
                if (sizeof($actividadeselegidas) > 0 && sizeof($servicioselegidos) > 0){
                    ...
                    for ($i = 0 ; $i < sizeof($actividadeselegidas) ; $i++){
                        for ($j = 0 ; $j < sizeof($servicioselegidos) ; $j++){
                            ...
                            $resultadoprecioservicio = mysql_query("SELECT * FROM servicios WHERE altertext = '$servicioselegidos[$j]'") ;
                ...
                $nuevoprecio = $facturaconcreta["preciototal"] + ($facturaaux->preciofinal * $intervalo->days) ;
            ...
            if ($numtipos > 0){
                ...
                if (isset($_SESSION["idreserva"])){
                    ...
                    $facturaconcreta = mysql_fetch_assoc($resultadofactura) ;
                    echo '<p><b>COSTE TOTAL DE LA RESERVA (SIN INCLUIR PROMOCIONES): '.$facturaconcreta["preciototal"].' 
                        EUROS</b></p>---------------------------------------------------------------------------------' ;
                            ...
                            if ($numactividades > 0){
                                ...
                                for ($i = 0 ; $i < $numactividades ; $i++){
                                    $actividadconcreta = mysql_fetch_assoc($resultadoactividades) ;
                                        ...
                                        <p>Precio: '.$actividadconcreta["precio"].' euros.</p>
                                        <p>ACTIVIDAD DISPONIBLE ACTUALMENTE.</p><br><br><br><br>

                                        <input type="checkbox" name="actividadelegir'.$i.'" value='.$actividadconcreta["altertext"].'>SELECCIONAR ESTA ACTIVIDAD</option></div>' ;
                                }
                            }
        ...
        <div id="primarycontainer">
            ...
            <?php
                ...
                if ($numfilashotel > 0){
                    ...
                    <p>Código postal: '.$elhotel["codigopostal"].'</p>
